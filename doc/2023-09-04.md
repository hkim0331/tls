...
9. VScode の左下、背景青で >< が斜めになってるとこをクリック、Connect to WSL を選ぶ。10数秒待つ。
...
something って？ アトムかリスト。

#### lambda

今ここで種明かしするのは教育的じゃない。自分で考えようってこと。

禅問答みたいでよければ、「lambda は関数」だな。

y=f(x) みたいな式、プログラム、動作、、、あとなんだ、があるとして、関数の部分はどれ？
...
やる気のない連中と重なる。
細かい、正確な定義を与えるのはこの本の主題ではないので
(lambda (x)...) は「関数の引数を x として」くらいに軽く考えたらどうかな。
読み進むにつれ、lambda の姿がはっきりしてくるから。

関数 lat? に戻ろう。
日本語で考えてみる。

- (null? x) が #t になる時、x は '()。要素を持たないが lat だ。空ラット。
- x の先頭要素がアトムなら、(lat? x) の真偽は (lat? (rest x)) の真偽に委ねられる。
  '(1 2 3 4 5) が lat かどうかは、1 がアトムゆえに、'(2 3 4 5) が lat かどうかにかかっている。
- x の先頭要素がアトムじゃないなら、(rest x) をチェックするまでもなく、x は lat ではないと即断できる。
  例えば、 x が '((1) 2 3 4 5 6 7) のケース。

2023-09-04 はこの辺で終わった。次に rember だったか member? だったか、そういう関数を作れって続きます。
